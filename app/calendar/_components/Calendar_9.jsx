  ...
  return (
    <div className="relative w-96 h-96 mx-auto rounded-full bg-gradient-to-tl from-gray-800 via-gray-700 to-gray-900 flex items-center justify-center overflow-hidden">
      ...
      <AnimatePresence mode="wait">
        <motion.div key={currentDate.toString()} initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0 }} transition={{ duration: 0.7 }} className="relative top-[-5%] left-[-6%] w-full h-full">
          {days.map(({ date, x, y }, index) => (
            <motion.button key={index} onClick={() => handleDateSelect(date)} style={{ position: "absolute", left: `calc(50% + ${x}px)`, top: `calc(50% - ${y}px)`, transform: "translate(-50%, -50%)" }} initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: index * 0.03, duration: 0.5 }} className={`w-12 h-12 flex items-center justify-center text-base font-medium rounded-full transition-transform duration-300 bg-gray-300 text-gray-800 hover:scale-110 ${selectedDate?.toDateString() === date.toDateString() ? "bg-gradient-to-br from-red-500 to-yellow-500 text-white ring-4 ring-gray-800" : "hover:bg-gray-400"}`}>
              {date.getDate()}
            </motion.button>
          ))}
        </motion.div>
