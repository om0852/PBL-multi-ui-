...
const RadialCircularCalendar = ({ initialDate, onSelectDate }) => {
  ...
  return (
    <div className="relative w-96 h-96 mx-auto rounded-lg bg-gradient-to-tr from-teal-500 via-yellow-500 to-pink-500 flex items-center justify-center overflow-hidden">
          ...
          {days.map(({ date, x, y }, index) => (
            <motion.button
              ...
              className={`w-12 h-12 flex items-center justify-center text-base font-medium rounded-full transition-transform duration-300
                bg-indigo-400 text-gray-800 hover:scale-110
                ${
                  selectedDate?.toDateString() === date.toDateString()
                    ? "bg-gradient-to-br from-pink-500 to-yellow-700 text-white ring-4 ring-teal-500 animate-pulse"
                    : "hover:bg-indigo-500"
                }`}
            >
              {date.getDate()}
